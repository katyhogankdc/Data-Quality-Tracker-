...
FROM
	...
	LEFT JOIN 
		...
		AND E.SCHOOLID != 1011  -- NE does not have homerooms
)  SUB
...
-----DED Not Marked as R in PowerSchool 
SELECT 
 I.STUDENTID AS STUDENT_NUMBER
,SCH.ABBREVIATION AS SCHOOL_NAME
,AC.ATT_CODE AS ERROR_GROUP
,CAST(i.issuets AS DATE) AS ERROR_DATE
,CASE 
    WHEN DATEPART(MM,CD.DATE_VALUE)>=7 THEN RIGHT(DATEPART(YY,CD.DATE_VALUE),2)+10
    WHEN DATEPART(MM,CD.DATE_VALUE)<=6 THEN RIGHT(DATEPART(YY,CD.DATE_VALUE),2)+9
    ELSE NULL 
...
,s.grade_level AS GRADE_LEVEL
,S.LASTFIRST AS LASTFIRST
,'https://kippdc.deanslistsoftware.com/penalties/'+CAST(P.PENALTYNAME AS VARCHAR) AS LINK
,CASE 
  WHEN p.penaltyname = 'Disciplinary Early Dismissal' 
     AND ac.att_code != 'R'
    THEN 'Missing Released Early Attendance Code in PS'
    ELSE NULL
END as ERROR
,'DeansList' AS SOURCESYSTEM
,27 ERRORID
FROM CUSTOM_DLINCIDENTS_RAW I
	...
	JOIN POWERSCHOOL.POWERSCHOOL_CALENDAR_DAY CD ON CD.DATE_VALUE = I.ISSUETS AND CD.SCHOOLID = SB.PSSCHOOLID
	JOIN POWERSCHOOL.POWERSCHOOL_SCHOOLS SCH ON SCH.SCHOOL_NUMBER = SB.PSSCHOOLID
	JOIN POWERSCHOOL.POWERSCHOOL_STUDENTS S ON S.STUDENT_NUMBER = I.STUDENTSCHOOLID
	JOIN (SELECT DISTINCT
        CASE 
        ...
        ,DATE_VALUE
        FROM POWERSCHOOL.POWERSCHOOL_CALENDAR_DAY CD
        ) CAL ON CAL.DATE_VALUE = I.ISSUETS
	LEFT OUTER JOIN POWERSCHOOL.POWERSCHOOL_ATTENDANCE A ON A.STUDENTID = S.STUDENT_NUMBER AND A.ATT_DATE = CD.DATE_VALUE AND ATT_MODE_CODE = 'ATT_ModeDaily'
    LEFT JOIN POWERSCHOOL.POWERSCHOOL_ATTENDANCE_CODE AC ON AC.ID = A.ATTENDANCE_CODEID --AND AC.SCHOOLID = SB.PSSCHOOLID
WHERE 1=1
and p.penaltyname = 'Disciplinary Early Dismissal'
AND ac.att_code != 'R'
...
--------------------------NO ATTENDANCE TAKEN IN DEANSLIST
SELECT
DL_ATT.STUDENTSCHOOLID AS STUDENT_NUMBER,
DL_ATT.SCHOOLNAME AS SCHOOL_NAME,
--SC.ABBREVIATION,
...
S.LASTFIRST AS LASTFIRST,
'https://powerschool.kippdc.org/admin/attendance/view/daily.html?frn=001' + CAST(S.DCID AS VARCHAR) AS LINK,
'Mismatch between Deanslist and Powerschool attendance records when no attendance taken' AS ERROR,
'PowerSchool' AS SOURCESYSTEM,
23 AS ERRORID
FROM CUSTOM.CUSTOM_DLATTENDANCE_RAW DL_ATT
JOIN POWERSCHOOL.POWERSCHOOL_STUDENTS S ON S.STUDENT_NUMBER = DL_ATT.STUDENTSCHOOLID
